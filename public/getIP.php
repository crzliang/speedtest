...
try {
    if(isset($_GET['isp'])){
        ...
        if (is_string($localIpInfo)) {
            $responseJson = formatResponse_simple($ip,$localIpInfo);
        }else{
            ...
            if(!is_null($r)){
                ...
                if(!is_null($r)){
                    ...
                    $responseJson = formatResponse_simple($ip);
                }
            }
        }
    }else{
    ...
    $responseJson = json_encode(['processedString'=>$ip ?: '0.0.0.0','rawIspInfo'=>'']);
}

// 清理任何意外输出（避免前面产生的空格破坏 JSON）
ob_end_clean();
if (!$responseJson || !is_string($responseJson)) {
    $responseJson = json_encode(['processedString'=>$ip ?: '0.0.0.0','rawIspInfo'=>'']);
}
echo $responseJson;
flush();

// Debug 时附带头部回显（不破坏 JSON 结构，可另用 ?debug=2 获取纯文本调试信息）
